"""
pages/flight_history.py — Browse, filter, edit, and export flight logs.
"""
...
# ---------------------------------------------------------------------------
# Edit dialog (Streamlit 1.35+ @st.dialog)
# ---------------------------------------------------------------------------
...
def _edit_dialog(log_id: int):
    ...
    # ---- init dynamic event rows for this dialog session ----
    ...
    if log.get("site_id"):
        cur_site = next((s["name"] for s in sites_list if s["id"] == log["site_id"]),
                        "Other / New Location")
    else:
        cur_site = "Other / New Location"
    ...
    # GCS: show dropdown if list exists, else free text
    ...
    if gcs_names:
        ...
        if gcs_sel == "Other (free text)":
            gcs_text = st.text_input("GCS Type (custom)",
    ...
    is_instructor= c7.checkbox("Acting as Instructor",
    ...
    if ev_list:
        ...
        hc[0].markdown("**Event Type**"); hc[1].markdown("**Day / Night**")
        hc[2].markdown("**Qty**");        hc[3].markdown("**Method**"); hc[4].markdown("**Del**")
...
def render():
    ...
    filtered = [
        l for l in logs
        if (f_mission    == "All" or l["mission_purpose"] == f_mission)
        and (f_role      == "All" or l["crew_role"]       == f_role)
        and (f_aircraft  == "All" or l["model_type"]      == f_aircraft)
        and (f_instructor == "All"
             or (f_instructor == "Yes" and l["is_instructor"])
             or (f_instructor == "No"  and not l["is_instructor"]))
    ]
    ...
    with st.expander("📤 Export", expanded=False):
        ...
        if exp_date_from:
            export_logs = [l for l in export_logs
                           if l["date"] >= exp_date_from.strftime("%Y-%m-%d")]
        if exp_date_to:
            export_logs = [l for l in export_logs
                           if l["date"] <= exp_date_to.strftime("%Y-%m-%d")]

        st.caption(f"{len(export_logs)} flight(s) will be exported.")
        dl_c1, dl_c2 = st.columns(2)

        if dl_c1.button("📄 Export PDF", use_container_width=True, key="exp_pdf_btn"):
            ...
            ok, msg = export_to_pdf(
                ...
                date_to=exp_date_to.strftime("%Y-%m-%d")     if exp_date_to   else None,
            )
            if ok:
                with open(fp, "rb") as fh:
                    dl_c1.download_button(
                        "⬇️ Download PDF", data=fh.read(),
        ...
        if dl_c2.button("📊 Export Excel", use_container_width=True, key="exp_xlsx_btn"):
            fp = os.path.join(EXPORT_DIR, "flight_logbook.xlsx")
            ok, msg = export_to_excel(export_logs, fp)
            if ok:
                with open(fp, "rb") as fh:
                    dl_c2.download_button(
                        "⬇️ Download Excel", data=fh.read(),
    ...
    for log in filtered:
        ...
        with st.expander(
            ...
            if act1.button("✏️ Edit",   key=f"edit_{log['id']}"):
                _edit_dialog(log["id"])

            if act2.button("🗑️ Delete", key=f"del_{log['id']}"):
